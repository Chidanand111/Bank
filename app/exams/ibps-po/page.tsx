...
export default async function IBPSPOPage() {
  const exam = await getExamBySlug('ibps-po');
  if (!exam) return notFound();

  const mockTests = await getMockTests('ibps-po');
      ...
      <div className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Badge variant="blue" size="sm">NATIONAL LEVEL</Badge>
                <Badge variant="slate" size="sm">11 PARTICIPATING BANKS</Badge>
              </div>
          ...
          <Link href="#available-tests">
            <Button variant="primary" size="lg" className="flex items-center gap-2 shadow-xs">
              <PlayCircle className="w-5 h-5" /> Start IBPS PO Mock
            </Button>
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2 text-xs">
          ...
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-100 space-y-1">
            ...
            <span className="text-lg font-bold text-slate-900">155 Qs (180 Mins)</span>
          </div>
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-100 space-y-1">
